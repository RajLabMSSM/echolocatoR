...
get_sample_size <- function(subset_DT,
                            ...
                            effective_ss=TRUE,
                            verbose=TRUE){
  messager("+ Preparing sample_size (N) column",v=verbose)
  if(!"N" %in% colnames(subset_DT)){
    if(is.null(sample_size)){
      if("N_cases" %in% colnames(subset_DT) & "N_controls" %in% colnames(subset_DT)){
        if(effective_ss){
          messager("++ Computing effective sample size.",v=verbose)
          subset_DT$N <- round(4.0 / (1.0/subset_DT$N_cases + 1.0/subset_DT$N_controls), digits = 0)
        }else {
          messager("++ Inferring sample size from max(N_cases) + max(N_controls):",sample_size,v=verbose)
          subset_DT$N <- subset_DT$N_cases + subset_DT$N_controls
        }
      } else {
        subset_DT$N <- NULL
        messager("++ `sample_size` not provided.",v=verbose)
      }
    } else {
      messager(paste0("++ Using `sample_size = ",sample_size,"` ",v=verbose) );
      subset_DT$N <- sample_size
    }
  } else {messager("+ `N` column already present in data.",v=verbose)}
  return(subset_DT)
}

...
#'
#' @param coloc_rds_files A list of full paths to coloc RDS results files generated by Jack's coloc scripts.
...
merge_coloc_results_each <- function(coloc_rds_files,
  ...
  if(file.exists(save_path) & force_new==F){
    ...
    merged_COLOC <- lapply(coloc_rds_files, function(f,
                                               ...
                                               .nThread=nThread,
                                               .verbose=verbose){
      i <- grep(f,coloc_rds_files)
      message(paste0("(",i,"/",length(coloc_rds_files),") "), f)
      load(f)

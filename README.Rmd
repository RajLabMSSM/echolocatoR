...
> **Level** indicates dependency depth: **4** = top-level pipeline (`echolocatoR`), **3** = analysis modules that depend on level 1-2 packages, **2** = mid-level utilities, **1** = base packages with no *echoverse* dependencies, **-** = non-pipeline packages. CI status reflects all runners combined. Click the badge to see per-runner results. Code coverage badges may show "unknown" until a `CODECOV_TOKEN` is configured.

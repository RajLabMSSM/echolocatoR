...
top_SNPs <- echodata::import_topSNPs(topSS = "~/Downloads/my_loci.xlsx") 
head(top_SNPs)
```
...
(without the need for manual column mapping) we must firs munge the sumstats:
  - 
...
dat_locus[,proportion_cases:=(NCAS/sum(NCAS+NCON,na.rm = TRUE))]
### Preview the data ####
...
Using stored LD example data instead.

```{r}
locus_dir <- file.path("~/Desktop/", top_SNPs_locus$Locus) 
